...
public sealed class AppSettingsConfigurationGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(combined, static (ctx, t) =>
        {
            ...
            ctx.AddSource(
                ...
                SourceText.From(EmitDependencyInjection(assemblyName, schema), Encoding.UTF8));
    ...
    private static string EmitDependencyInjection(string assemblyName, AppSettingsSchema schema)
    {
        ...
        sb.AppendLine("        global::System.ArgumentNullException.ThrowIfNull(configuration);");

        foreach (var section in schema.TopLevelSections.OrderBy(static s => s.SectionName, StringComparer.Ordinal))
        {
            var typeFqn = "global::" + section.TypeFullName;
            var getMethod = "Get" + ToPascalIdentifier(section.SectionName);
            sb.AppendLine($"        services.AddSingleton<{typeFqn}>(configuration.{getMethod}());");
        }
    ...
    private static string EmitConfigurationExtensions(string rootNamespace, AppSettingsSchema schema)
    {
        ...
        foreach (var section in schema.TopLevelSections.OrderBy(s => s.SectionName, StringComparer.Ordinal))
        {
            ...
            sb.AppendLine("    {");
            sb.AppendLine("        global::System.ArgumentNullException.ThrowIfNull(configuration);");
            sb.AppendLine($"        return Bind{section.TypeName}(configuration.GetSection(\"{sectionLiteral}\"));");
            sb.AppendLine("    }");
            sb.AppendLine();
        }
        ...
        foreach (var type in schema.Types)
        {
            EmitBindType(sb, rootNamespace, type);
            sb.AppendLine();
        }
    ...
    private static void EmitBindType(StringBuilder sb, string rootNamespace, ConfigType type)
    {
        var typeName = type.FullName.Substring(rootNamespace.Length + 1);
        sb.AppendLine($"    private static global::{type.FullName} Bind{typeName}(global::Microsoft.Extensions.Configuration.IConfigurationSection section)");
        sb.AppendLine("    {");
        sb.AppendLine($"        return new global::{type.FullName}");
        sb.AppendLine("        {");

        foreach (var prop in type.Properties.OrderBy(static p => p.Name, StringComparer.Ordinal))
        {
            sb.AppendLine($"            {prop.Name} = {EmitBindExpression(rootNamespace, typeName, prop)},");
        }

        sb.AppendLine("        };");
        sb.AppendLine("    }");

        foreach (var prop in type.Properties.Where(static p => p.CSharpType.StartsWith("global::System.Collections.Generic.List<", StringComparison.Ordinal)))
        {
            EmitBindList(sb, rootNamespace, typeName, prop);
        }
    }

    private static string EmitBindExpression(string rootNamespace, string typeName, ConfigProperty prop)
    {
        var sectionExpression = $"section.GetSection(\"{EscapeStringLiteral(prop.JsonKey)}\")";
        var nestedType = TryGetNestedGeneratedTypeFullName(rootNamespace, prop.CSharpType);
        if (nestedType != null)
        {
            return "Bind" + nestedType.Substring(rootNamespace.Length + 1) + "(" + sectionExpression + ")";
        }

        if (prop.CSharpType.StartsWith("global::System.Collections.Generic.List<", StringComparison.Ordinal))
        {
            return "Bind" + typeName + prop.Name + "(" + sectionExpression + ")";
        }

        return GetValueReaderName(prop.CSharpType) + "(" + sectionExpression + ")";
    }

    private static void EmitBindList(StringBuilder sb, string rootNamespace, string typeName, ConfigProperty prop)
    {
        var elementType = TryGetListElementType(prop.CSharpType) ?? "global::System.String";
        sb.AppendLine($"    private static {prop.CSharpType} Bind{typeName}{prop.Name}(global::Microsoft.Extensions.Configuration.IConfigurationSection section)");
        sb.AppendLine("    {");
        sb.AppendLine($"        var values = new {prop.CSharpType}();");
        sb.AppendLine("        foreach (var item in section.GetChildren())");
        sb.AppendLine("        {");
        var nestedType = TryGetNestedGeneratedTypeFullName(rootNamespace, elementType);
        var valueExpression = nestedType != null
            ? "Bind" + nestedType.Substring(rootNamespace.Length + 1) + "(item)"
            : GetValueReaderName(elementType) + "(item)";
        sb.AppendLine($"            values.Add({valueExpression});");
        sb.AppendLine("        }");
        sb.AppendLine("        return values;");
        sb.AppendLine("    }");
    }

    private static string GetValueReaderName(string type) => type switch
    {
        "global::System.String" => "ReadString",
        "global::System.Boolean" => "ReadBoolean",
        "global::System.Int32" => "ReadInt32",
        "global::System.Int64" => "ReadInt64",
        "global::System.Double" => "ReadDouble",
        "global::System.Decimal" => "ReadDecimal",
        _ => "ReadString"
    };

    private static void EmitValueReaders(StringBuilder sb)
    {
        sb.AppendLine("    private static string ReadString(global::Microsoft.Extensions.Configuration.IConfigurationSection section)");
        sb.AppendLine("        => section.Value ?? throw MissingValue(section.Path);");
        sb.AppendLine();
        sb.AppendLine("    private static bool ReadBoolean(global::Microsoft.Extensions.Configuration.IConfigurationSection section)");
        sb.AppendLine("        => bool.TryParse(ReadString(section), out var value) ? value : throw InvalidValue(section.Path, \"Boolean\");");
        sb.AppendLine();
        sb.AppendLine("    private static int ReadInt32(global::Microsoft.Extensions.Configuration.IConfigurationSection section)");
        sb.AppendLine("        => int.TryParse(ReadString(section), global::System.Globalization.NumberStyles.Integer, global::System.Globalization.CultureInfo.InvariantCulture, out var value) ? value : throw InvalidValue(section.Path, \"Int32\");");
        sb.AppendLine();
        sb.AppendLine("    private static long ReadInt64(global::Microsoft.Extensions.Configuration.IConfigurationSection section)");
        sb.AppendLine("        => long.TryParse(ReadString(section), global::System.Globalization.NumberStyles.Integer, global::System.Globalization.CultureInfo.InvariantCulture, out var value) ? value : throw InvalidValue(section.Path, \"Int64\");");
        sb.AppendLine();
        sb.AppendLine("    private static double ReadDouble(global::Microsoft.Extensions.Configuration.IConfigurationSection section)");
        sb.AppendLine("        => double.TryParse(ReadString(section), global::System.Globalization.NumberStyles.Float | global::System.Globalization.NumberStyles.AllowThousands, global::System.Globalization.CultureInfo.InvariantCulture, out var value) ? value : throw InvalidValue(section.Path, \"Double\");");
        sb.AppendLine();
        sb.AppendLine("    private static decimal ReadDecimal(global::Microsoft.Extensions.Configuration.IConfigurationSection section)");
        sb.AppendLine("        => decimal.TryParse(ReadString(section), global::System.Globalization.NumberStyles.Number, global::System.Globalization.CultureInfo.InvariantCulture, out var value) ? value : throw InvalidValue(section.Path, \"Decimal\");");
        sb.AppendLine();
        sb.AppendLine("    private static global::System.InvalidOperationException MissingValue(string path)");
        sb.AppendLine("        => new(\"Required configuration value is missing: \" + path);");
        sb.AppendLine();
        sb.AppendLine("    private static global::System.InvalidOperationException InvalidValue(string path, string type)");
        sb.AppendLine("        => new(\"Configuration value at '\" + path + \"' is not a valid \" + type + \".\");");
    }
